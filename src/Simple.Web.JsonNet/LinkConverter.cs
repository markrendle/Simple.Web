namespace Simple.Web.JsonNet
{
    ...
    {
        internal static readonly MethodInfo Serialize = typeof(JsonSerializer).GetMethod("Serialize",
                                                                                         new[] { typeof(JsonWriter), typeof(object) });

        internal static readonly MethodInfo WritePropertyName = typeof(JsonWriter).GetMethod("WritePropertyName", new[] { typeof(string) });

        private static readonly ConcurrentDictionary<Type, JsonConverter[]> ConverterSets =
        ...
        public static JsonConverter[] CreateForGraph(Type type,
                                                     HashSet<Type> knownTypes,
                                                     Func<object, IEnumerable<object>> linkEnumerator,
        ...
        private static JsonConverter[] CreateForGraphImpl(Type type,
                                                          HashSet<Type> knownTypes,
                                                          Func<object, IEnumerable<object>> linkEnumerator,
                                                          IContractResolver contractResolver)
        {
