namespace Simple.Web.CodeGeneration
{
    ...
    internal class AsyncPipeline
    {
        ...
        private static Task<bool> ContinueWithAction<THandler>(Task<bool> task,
        ...
        private static Task<bool> ContinueWithAsyncBlock<THandler>(Task<bool> task,
                                                                   Func<THandler, IContext, Task<bool>> continuation,
                                                                   IContext context,
                                                                   THandler handler)
        {
            return task.ContinueWith(t =>
        ...
        private static Task<bool> ContinueWithAsyncHandler<THandler>(Task<bool> task,
                                                                     Func<THandler, IContext, Task<Status>> continuation,
                                                                     IContext context,
                                                                     THandler handler)
        {
            return task.ContinueWith(t =>
                                     {
                                         if (t.Result)
                                         {
                                             return continuation(handler, context).ContinueWith(ht =>
                                                                                                {
        ...
        private static Task<bool> ContinueWithHandler<THandler>(Task<bool> task,
                                                                Func<THandler, IContext, Status> continuation,
                                                                IContext context,
                                                                THandler handler)
        {
            return task.ContinueWith(t =>
